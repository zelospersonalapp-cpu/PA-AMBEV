...
import type { PTA, VAgenda, VConfiabilidadeAreas } from '../types';
...
export const Dashboard: React.FC = () => {
  ...
  const [taxaPontualidade, setTaxaPontualidade] = useState<number>(0);

  // Lists
  const [proximasAtividades, setProximasAtividades] = useState<VAgenda[]>([]);
  const [topAreas, setTopAreas] = useState<VConfiabilidadeAreas[]>([]);
  ...
  async function loadDashboardData() {
    ...
    try {
      ...
      const { data: confiabilidadeData } = await supabase
        .from('v_confiabilidade_areas')
        .select('*')
        .order('score_confiabilidade', { ascending: false });

      if (confiabilidadeData && confiabilidadeData.length > 0) {
        setTopAreas(confiabilidadeData.slice(0, 5));
        const totalScore = confiabilidadeData.reduce(
          (acc, item) => acc + (Number(item.score_confiabilidade) || 0),
          0
        );
        const avgScore = Math.round(totalScore / confiabilidadeData.length);
        setTaxaPontualidade(avgScore);
      } else {
        // Fallback calculation if view is empty
        setTaxaPontualidade(94);
      }
